...
class ReserveAppointmentForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if service_instance:
            start = service_instance.start_reservation_date
            end = service_instance.finish_reservation_date
            if start and end and start <= end:
                current_date = start
                from jdatetime import timedelta
                valid_dates = []
                while current_date <= end:
                    valid_dates.append((current_date.strftime("%Y-%m-%d"), current_date.strftime("%Y/%m/%d")))
                    current_date += timedelta(days=1)
                self.fields['selected_date'].choices = valid_dates
